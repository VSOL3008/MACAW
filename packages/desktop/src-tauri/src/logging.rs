...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("macaw-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("macaw_lib=debug,macaw_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("macaw_lib=info,macaw_desktop=info,sidecar=info")
        }
    });
