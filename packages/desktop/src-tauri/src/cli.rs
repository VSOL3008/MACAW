...
const CLI_INSTALL_DIR: &str = ".macaw/bin";
const CLI_BINARY_NAME: &str = "macaw";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("macaw-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let install = option_env!("MACAW_INSTALL_URL");
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.macaw/bin/macaw\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                match install {
                    Some(url) => format!(
                        "  curl -fsSL {} | bash -s -- --version {} --no-modify-path",
                        shell_escape(url),
                        shell_escape(&version)
                    ),
                    None => {
                        "  echo 'MACAW_INSTALL_URL is not configured for WSL bootstrap' >&2; exit 1"
                            .to_string()
                    }
                },
                "fi".to_string(),
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "macaw".to_string()),
