...
export async function createMenu(trigger: (id: string) => void) {
  ...
  await initI18n()
  const docs = import.meta.env.VITE_MACAW_DOCS_URL
  const support = import.meta.env.VITE_MACAW_SUPPORT_URL
  const feedback = import.meta.env.VITE_MACAW_FEEDBACK_URL
  const bug = import.meta.env.VITE_MACAW_BUG_URL
