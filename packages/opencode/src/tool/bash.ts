...
export const BashTool = Tool.define("bash", async () => {
  ...
  return {
    ...
    async execute(params, ctx) {
      ...
      if (!PS.has(name) && /^\s*(powershell(\.exe)?|pwsh(\.exe)?)\b/i.test(params.command)) {
        throw new Error(
          "Do not launch PowerShell through bash. bash expands $variables before PowerShell sees them, so `$x = 1` becomes `= 1`. For Outlook mail use the `outlook` tool; for any other PowerShell use the `powershell` tool (its `script` parameter is passed verbatim via -EncodedCommand).",
        )
      }
