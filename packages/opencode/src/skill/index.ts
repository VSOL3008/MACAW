...
export namespace Skill {
  ...
  const MEMORY_SKILLS_SUBDIR = "skills"
  ...
  const loadSkills = Effect.fnUntraced(function* (
    ...
    if (!Flag.OPENCODE_DISABLE_EXTERNAL_SKILLS) {
      ...
      const memory = path.join(Memory.root(), MEMORY_SKILLS_SUBDIR)
      if (yield* fsys.isDir(memory)) {
        yield* scan(state, bus, memory, SKILL_PATTERN, { scope: "memory" })
      }
    }
