...
test.skip("mode cost preserves over-200k pricing from base model", () => {
  // Legacy model-dev provider conversion is not used in Ollama-only mode.
})
...
test("ollama discovers mai-ui models as vision capable", async () => {
  const orig = globalThis.fetch
  globalThis.fetch = mock((url: string | URL | Request) => {
    expect(url.toString()).toBe("http://ollama.test/api/tags")
    return Promise.resolve(
      new Response(
        JSON.stringify({
          models: [
            {
              name: "ahmadwaqar/mai-ui:8b",
              modified_at: "2026-01-09T08:44:59.000Z",
              details: {
                family: "qwen3_vl",
                families: ["qwen3_vl"],
              },
  ...
  try {
    await Instance.provide({
      directory: tmp.path,
      init: async () => {
        Env.set("OLLAMA_BASE_URL", "http://ollama.test/v1")
      },
      fn: async () => {
        const providers = await Provider.list()
        const model = providers[ProviderID.ollama].models["ahmadwaqar/mai-ui:8b"]
        expect(model.capabilities.input.image).toBe(true)
        expect(model.capabilities.attachment).toBe(true)
      },
    })
