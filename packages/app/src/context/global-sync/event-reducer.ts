...
import { FallbackRegistry } from "@/utils/fallback"
...
export function applyDirectoryEvent(input: {
  ...
  switch (event.type) {
    ...
    case "session.status": {
      ...
      if (props.status?.type === "idle") FallbackRegistry.clear(props.sessionID)
      else FallbackRegistry.touch(props.sessionID)
      break
    }
    case "message.updated": {
      const info = clean((event.properties as { info: Message }).info)
      FallbackRegistry.touch(info.sessionID)
    ...
    case "message.part.updated": {
      ...
      if (part.sessionID) FallbackRegistry.touch(part.sessionID)
